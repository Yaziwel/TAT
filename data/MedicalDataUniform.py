...
class DataSampler:
    def __init__(self, dataloader_list):
        ...
        self.data_iters = [iter(dl) for dl in self.dataloader_list]
    ...
    def __next__(self):
        ...
        for i, data_iter in enumerate(self.data_iters):
            try:
                batch_lq, batch_hq, batch_mod = next(data_iter)
            except StopIteration:
                self.data_iters[i] = iter(self.dataloader_list[i])
                batch_lq, batch_hq, batch_mod = next(self.data_iters[i])

            for lq, hq, mod in zip(batch_lq, batch_hq, batch_mod):
                samples.append((lq, hq, mod))

        random.shuffle(samples)

        lq_batch, hq_batch, mod_batch = zip(*samples)

        lq_batch = torch.stack(lq_batch, dim=0)
        hq_batch = torch.stack(hq_batch, dim=0)

        return lq_batch, hq_batch, list(mod_batch)
